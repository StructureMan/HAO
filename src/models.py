...
class GAT_AD(nn.Module):
    def __init__(self, feats):
        ...
        self.edge_index = torch.stack([torch.tensor(src_ids), torch.tensor(dst_ids)]).to(device)
        self.gat = torch_geometric.nn.GATConv(in_channels=feats, out_channels=feats, heads=1).to(device)

        self.fcn = nn.Sequential(nn.Linear((feats + 1) * feats, feats * feats),
                                 nn.SiLU()).to(device)

    def forward(self, data):
        data = data.view(self.n_window, self.n_feats)
        data_r = torch.cat((torch.zeros(1, self.n_feats).to(device), data))
        data_r = data_r.to(torch.float64)
        feat_r = self.gat(data_r, self.edge_index.to(device))
        out = self.fcn(feat_r.view(-1))
...
class DGINet(nn.Module):
    def __init__(self, feats, n_windows, is_change_branch=False):
        super(DGINet, self).__init__()
        self.name = 'DGINet'
        self.lr = 0.0001
        self.n_feats = feats
        self.n_window = n_windows
        ...
        self.edge_index = torch.stack([torch.tensor(src_ids), torch.tensor(dst_ids)]).to(device)
        self.gat_head = 2
        self.gat = torch_geometric.nn.GATConv(in_channels=self.n_window, out_channels=self.n_window,
                                              heads=self.gat_head).to(
            device)

        self.fcn = nn.Sequential(nn.Linear((feats + 1) * self.n_window * self.gat_head, self.n_window * feats),
                                 nn.SiLU()).to(device)
        self.num_head = 6
        self.n = self.n_feats * self.n_window
        self.atts = [nn.Sequential(nn.Linear(self.n, self.n_window * self.num_head * feats).to(device),
                                   nn.PReLU().to(device)) for i in range(3)]

        self.atts = nn.ModuleList(self.atts).to(device)
        self.MultiAttention = MultiHeadAttention(d_model=self.n_feats * self.num_head, num_heads=self.num_head).to(
            device)

        self.fcn_ma = nn.Sequential(nn.Linear(self.num_head * self.n_feats, feats).to(device),
                                    nn.SELU(True).to(device))

        self.fcn_all = nn.Sequential(nn.Linear(feats * self.n_window, feats * self.n_window).to(device),
                                     nn.ReLU(True).to(device))
        self.num_convlayer = 1
        self.conv_lstm = ConvLSTM(input_dim=1, hidden_dim=self.n_hidden, kernel_size=(3, 3),
                                  num_layers=self.num_convlayer,
                                  batch_first=True,
                                  return_all_layers=False).to(device)
        self.conv_lstm = nn.GRU(feats, self.n_feats).to(device)
        self.fcn_out = nn.Sequential(
            nn.Linear(self.n_feats * self.n_window * self.n_hidden, self.n_feats * self.n_window), nn.SiLU()).to(
            device)

        self.fcn_out_con = nn.Sequential(
            nn.Linear(self.n_feats * self.n_window * self.n_hidden * 3, self.n_feats * self.n_window), nn.SiLU()).to(
            device)


        self.n_latent = 8
        self.n_hidden_vae = 32
        self.encoder = nn.Sequential(
            nn.Linear(self.n_feats, self.n_hidden_vae), nn.PReLU(),
            nn.Linear(self.n_hidden_vae, self.n_hidden_vae), nn.PReLU(),
            nn.Flatten(),
            nn.Linear(self.n_hidden_vae, 2 * self.n_latent)
        ).to(device)
        self.decoder = nn.Sequential(
            nn.Linear(self.n_latent, self.n_hidden_vae), nn.PReLU(),
            nn.Linear(self.n_hidden_vae, self.n_hidden_vae), nn.PReLU(),
            nn.Linear(self.n_hidden_vae, self.n_feats), nn.Sigmoid(),
        ).to(device)
        self.beta = 0.01

        self.fcn_vae = nn.Sequential(
            nn.Linear(self.n_feats * self.n_window, self.n_feats * self.n_window), nn.SiLU()).to(device)

    def forward(self, data):
        ma = data
        data = data.view(self.n_window, self.n_feats).t()
        data_r = torch.cat((torch.zeros(1, self.n_window).to(device), data))
        feat_r = self.gat(data_r, self.edge_index)
        out_gat = self.fcn(feat_r.view(-1))
        # 多头注意力
        q = self.atts[0](ma.view(-1)).reshape(-1, self.n_window, self.num_head * self.n_feats)
        k = self.atts[1](ma.view(-1)).reshape(-1, self.n_window, self.num_head * self.n_feats)
        v = self.atts[2](ma.view(-1)).reshape(-1, self.n_window, self.num_head * self.n_feats)
        g, ats = self.MultiAttention(query=q, key=k, value=v)
        out_ma = self.fcn_ma(g)
        out_pre = self.fcn_all(out_gat.view(-1) + out_ma.view(-1))
        input = out_pre.view(self.n_window, self.n_feats)
        lstm_out, hiddens = self.conv_lstm(input)
        # res = lstm_out[0].view(-1)
        res = lstm_out.view(-1)
        out = self.fcn_out(res)
        lstm_out = out
        out = torch.cat((out_gat.view(-1), out_ma.view(-1), lstm_out.view(-1)))
        out_con = self.fcn_out_con(out)
        # out_con = lstm_out
        h = self.encoder(out_con.view(-1, self.n_feats))
        ...
        ## Decoder
        att = self.decoder(x)
        x = self.fcn_vae(att.view(-1))
        x = torch.nan_to_num(x, nan=0.)
        return x.view(self.n_window, self.n_feats), mu.reshape(-1), logvar.reshape(-1), att
...
class GRN(nn.Module):
    def __init__(self, feats):
        ...
        self.gru = GRU_AD(feats=feats).to(device)

        self.out = nn.Sequential(
            nn.Linear(2 * self.n_feats * self.n_window, self.n_feats * self.n_window), nn.LeakyReLU(True),
            nn.Linear(self.n_feats * self.n_window, self.n_feats * self.n_window), nn.Sigmoid(),
        ).to(device)
    ...
    def forward(self, data):
        ...
        # GRU
        time_feature = self.gru(data)
        # Pass through a FCN
        x = self.fcn(feat_r)
        new_featue = torch.concat([time_feature.view(-1), x.view(-1)])
        out = self.out(new_featue)
        return out
